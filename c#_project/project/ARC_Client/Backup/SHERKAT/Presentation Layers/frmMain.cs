...
namespace Mehr.Presentation_Layers
{
    public partial class frmMain : Form
    {
        ...
        //        catch (Exception ex)
        //        {
        //            MessageBox.Show(ex.ToString()+" تهیه فایل پشتیبان با مشکل مواجه شد ", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
        //        }
        ...
        private void تنظیماتسرورToolStripMenuItem_Click(object sender, EventArgs e)
        {
            bool IsOpen = false;

            foreach (Form f in Application.OpenForms)
            {
                if (f.GetType() == typeof(frmSelectServers))
                {
                    IsOpen = true;
                    f.Focus();
                    break;
            ...
            if (IsOpen == false)
            {
                frmSelectServers fsdv = new frmSelectServers();
                fsdv.MdiParent = this;
                fsdv.Show();
            }
        ...
        private void دفتریToolStripMenuItem3_Click(object sender, EventArgs e)
        {
            ...
            foreach (Form f in Application.OpenForms)
            {
                if (f.GetType() == typeof(frmAzmayeshHisView))
                {
                    IsOpen = true;
                    f.Focus();
                    break;
        ...
        private void واقعیToolStripMenuItem3_Click(object sender, EventArgs e)
        {
            ...
            foreach (Form f in Application.OpenForms)
            {
                if (f.GetType() == typeof(frmRealAzmayeshView))
                {
                    IsOpen = true;
                    f.Focus();
                    break;
        ...
        private void تعویضکاربریToolStripMenuItem_Click(object sender, EventArgs e)
        {
            bazrasVorood(); 
            acc acnt = new acc();
            if (acnt.checkpass().Rows.Count > 0)
        ...
        private void نمایشبانکداروییمخاطبینToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            bool IsOpen = false;
